...
def vot_comment(div):
    ...
    comments = div.xpath(
        '../div/span[@class = "stats-comments"]/a/i/text()')[0]
    vots_comments = '\n好笑数：' + vote + '\n评论数：' + comments + '\n'
...
def main_program():
    ...
    for page in range(0, end_page):
        url = 'https://www.qiushibaike.com/page/%s/' % page
        req = requests.get(url)
        html = req.text
        tree = etree.HTML(html)
        ...
        for div in first:
            ...
            if auther == []:
                ...
                if gender != []:
                    age = div.xpath(
                        './div[@class = "articleGender manIcon"]/text()')[0]
                    a = '\n作者：' + auther[0].strip('\n') + '\n性别：男，年龄：' + age
                    dz.append(a)
                else:
                    age = div.xpath(
                        './div[@class = "articleGender womenIcon"]/text()')[0]
                    a = '\n作者：' + auther[0].strip('\n ') + '\n性别：女，年龄：' + age
                    dz.append(a)
                c_txt = contents(div)
                dz.append(c_txt)
        ...
        with open('qiubaiduanzi.txt', 'w', encoding='utf8') as f:
            for line in dz:
                print(line)
                f.write(line)
